...
export const BUNDLED_FONTS = new Set([
  ...
  'Noto Serif CJK SC',
  'PrismOffice Sans KR',
  'PrismOffice Serif KR',
  'PrismOffice Fullwidth TC',
  'PrismOffice Songti SC',
  'Carlito GO',
  'Caladea',
  'Liberation Serif',
  'Liberation Sans',
...
export function cssFontFamily(font: string): string {
  ...
  // 'PrismOffice Box Drawing' (fonts.css): U+2500 rules would otherwise fall to
  // the fullwidth CJK subset and overflow the column
  const BOX = 'PrismOffice Box Drawing'
  ...
  // 'PrismOffice Songti SC' (fonts.css local() alias of Songti SC): macOS Chromium
  ...
  if (f.includes('simsun') || f.includes('宋体') || f.includes('nsimsun')) {
    return `${chain(font, 'PrismOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF)},serif`
  }
  ...
  // 'PrismOffice *' entries are CJK-only local() aliases (fonts.css): the underlying
  // system faces draw Cyrillic/Greek fullwidth, so those scripts must pass through
  const JA_SANS = ['Yu Gothic', 'PrismOffice Hiragino Sans', 'Meiryo', 'Noto Sans JP']
  const JA_SERIF = [
    'Yu Mincho',
    'PrismOffice Hiragino Mincho',
    'PrismOffice MS Mincho',
    'Noto Serif JP',
  ]
  const KO_SANS = ['Malgun Gothic', 'PrismOffice Sans KR', 'Apple SD Gothic Neo', 'Noto Sans KR']
  const KO_SERIF = ['PrismOffice Batang', 'PrismOffice Serif KR', 'PrismOffice Myungjo', 'Noto Serif KR']
  const TC_SANS = ['Microsoft JhengHei', 'PingFang TC', 'PrismOffice Heiti TC', 'Noto Sans TC']
  // 'PrismOffice Fullwidth TC' (fonts.css): fullwidth U+FF0D/FF0F/FF3C/FF3F/FF5E whose Songti TC glyphs look half-width
  const TC_SERIF = ['PrismOffice MingLiU', 'PrismOffice Fullwidth TC', 'Songti TC', 'Noto Serif TC']
  const SC_SANS = ['PingFang SC', 'Microsoft YaHei', CJK_SANS]
  const SC_SERIF = ['PrismOffice Songti SC', 'STSong', 'SimSun', CJK_SERIF]
  ...
  if (/nyala|ebrima|abyssinica|ethiopic/i.test(nfkc)) {
    return `${chain(font, 'PrismOffice Ethiopic')},sans-serif`
  }
