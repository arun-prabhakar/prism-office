...
const STATIC_FORM_FILLS_KEY = PDFName.of('PrismOfficeStaticFormFills')
...
function setVisualSignatureMetadata(annot: PDFDict, fieldName: string | undefined): void {
  if (!fieldName) return
  annot.set(PDFName.of('PrismOfficeFormField'), PDFHexString.fromText(fieldName))
  annot.set(
    PDFName.of('Contents'),
...
function addDrawing(
  ...
  if (d.kind === 'note') {
    ...
    annot.set(PDFName.of('T'), PDFHexString.fromText(d.author || 'PrismOffice'))
