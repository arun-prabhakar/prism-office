...
describe('resolveDefaultSaveDir', () => {
  ...
  it('creates and returns the fallback when nothing is configured', () => {
    const fallback = join(root, 'Documents', 'PrismOffice')
...
describe('configuredDefaultSaveDir', () => {
  ...
  it('falls back to <Documents>/PrismOffice without a setting', () => {
    const userData = join(root, 'userData')
    const documents = join(root, 'Documents')
    mkdirSync(userData, { recursive: true })
    const app = {
    ...
    expect(configuredDefaultSaveDir(app)).toBe(join(documents, 'PrismOffice'))
    expect(existsSync(join(documents, 'PrismOffice'))).toBe(true)
  })
})
