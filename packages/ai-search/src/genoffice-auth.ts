/**
 * PrismOffice's own Genspark identity: device-code login (office_addin_auth,
 ...
 * subprocesses) to PrismOffice. Stored in ~/.genoffice/auth.json, deliberately
...
/** The PrismOffice-named api key; '' when not signed in. Cached (invalidated by login/logout). */
...
/**
 * Starts the PrismOffice device-code login, cancelling a previous in-flight one
...
/**
 * Signs out of PrismOffice only: best-effort server-side revoke of the
