...
for (const xlsx of files) {
  ...
  try {
    ...
    // round-trip), redo only the PrismOffice shot and the diff.
...
<table><tr><td>Excel<br><img src="${rel(r.ref)}"></td><td>PrismOffice Sheets<br><img src="${rel(r.ours)}"></td><td>diff<br><img src="${rel(r.diff)}"></td></tr></table>`,
  )
  .join('')}
`
